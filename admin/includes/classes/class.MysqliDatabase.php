...
/**
* Configuration data for database connection
*/
...
$ConnectionString="server=".DB_HOST."; database=".DB_NAME."; username=".DB_USER."; password=".DB_PASSWORD."; port=; socket=";
...
class MysqliDatabase extends mysqli
{
	...
	*/
	private function __construct($ThrowExceptions = true)
	{
		...
		@$this->connect(empty($CS['server']) ? 'localhost' : $CS['server'],
							 ...
							 empty($CS['database']) ? 'itg' : $CS['database'],
    ...
    */
...

	{
		...
		/**
		 * A clone constructor
		 * It cannot permit to create a copy of Instance 
...

	{
		...
		trigger_error("Clone is not allowed here.", E_USER_ERROR);

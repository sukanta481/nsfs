...
<div class="x_panel">
	...
	<div class="x_content">
		...
		<form id="edit_trip_form" action="" method="post" name="edit_trip_form" class="form-horizontal form-label-left" novalidate enctype="multipart/form-data">
			...
			<div class="item form-group">
				<label class="control-label col-md-3 col-sm-3 col-xs-12">Whight (kg):</label>
				<div class="col-md-6 col-sm-6 col-xs-12">
      ...
      <div class="item form-group">
          ...
          <div class="col-md-6 col-sm-6 col-xs-12">
              <select class="form-control col-md-7 col-xs-12" name="status" onchange="change_status(this.value);">
                  ...
                  <option value="Created" <?php if($get_shipping_row['status']=='Created'){ echo "selected";}?>>Shipment Created</option>
                  <option value="Picked Up" <?php if($get_shipping_row['status']=='Picked Up'){ echo "selected";}?>>Picked Up</option>
                  <option value="In Transit" <?php if($get_shipping_row['status']=='In Transit'){ echo "selected";}?>>In Transit</option>
                  <option value="Delay" <?php if($get_shipping_row['status']=='Delay'){ echo "selected";}?>>Delay</option>
                  <option value="Out for Delivery" <?php if($get_shipping_row['status']=='Out for Delivery'){ echo "selected";}?>>Out for Delivery</option>
